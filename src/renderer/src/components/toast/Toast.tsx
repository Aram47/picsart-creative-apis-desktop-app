...
const Toast: React.FC<IToast> = ({ text, type, duration = 3000 }) => {
  const classes = useStyles() // Load styles
  // const iconPath = `/src/assets/icons/${type}.svg`
  ...
  return (
    <div
      className={`${classes.toastContainer} ${
        classes[`toastContainer${type.charAt(0).toUpperCase() + type.slice(1)}`]
      } ${closeClass ? classes.closeToast : ''}`}
    >
